...
7. **`pipeline.py`** — `run_pipeline()`: 19 steps across 4 phases (Phase 1 INGEST: import→dedupe→clean_text→fix_mismatches→parse; Phase 2 ANALYZE: mixxx_import→bpm→key→mixxx_blobs→energy→embed→cues; Phase 3 ENRICH: enrich_meta→enrich_genre→clean_genres; Phase 4 SYNC: cluster→crates→sync→report); `phase=` param runs a single phase; one backup at start; per-step error isolation; lazy-imports embed/cluster/cues for graceful degradation
