...
| Command | Description |
|---|---|
| `pipeline` | Primary daily workflow: chains all 19 steps in 4 phases (ingest→analyze→enrich→sync); `--apply`, `--skip-<step>`, `--music-dir`, `--phase ingest\|analyze\|enrich\|sync\|quick` (`quick` = import+clean+dedupe+sync, no analysis) |
...
7. **`pipeline.py`** — `run_pipeline()`: 19 steps across 4 phases (Phase 1 INGEST: import→dedupe→clean_text→fix_mismatches→parse; Phase 2 ANALYZE: mixxx_import→bpm→key→energy→embed→cues; Phase 3 ENRICH: enrich_meta→enrich_genre→clean_genres; Phase 4 SYNC: cluster→crates→sync→**mixxx_blobs**→report); **Phase quick** = import→dedupe→clean_text→fix_mismatches→parse→**mixxx_import**→crates→sync→mixxx_blobs→report (everyday workflow: pulls Mixxx's detected BPMs, no librosa); `mixxx_blobs` runs in Phase 4 after `sync` so new tracks exist in Mixxx before BeatGrid BLOBs are written; `phase=` param runs a single phase; one backup at start; per-step error isolation; lazy-imports embed/cluster/cues for graceful degradation
...
15. **`import_mixxx_analysis.py`** — reads Mixxx's own BPM/key analysis from Mixxx DB and imports into MultiDJ tracks table via path matching; dry-run by default
16. **`models.py`** — `LibrarySummary` dataclass
17. **`adapters/base.py`** — `SyncAdapter` ABC (`import_all`, `push_track`, `full_sync`)
18. **`adapters/mixxx.py`** — `MixxxAdapter`: import from Mixxx; push tracks + crates + cues back on sync; `_push_cues_to_mixxx()` writes intro (slot 0, blue) / drop (slot 1, red) / outro (slot 2, green)
19. **`adapters/directory.py`** — `DirectoryAdapter`: imports audio files from filesystem paths
20. **Command modules** (`scan`, `audit`, `clean`, `analyze`, `parse`, `enrich`, `crates`, `dedupe`) — pure business logic, read-only unless `--apply`
...
- `Vibe/` crates are auto-generated by `cluster vibe`; cleared and rebuilt on each apply run. Noise tracks (HDBSCAN label -1) go to `Vibe/Unclassified`.
...
- Config (`~/.multidj/config.toml`) controls crate dimensions, stores Mixxx DB path (`[mixxx].path`), and controls pipeline behavior; Mixxx commands fall back to `[mixxx].path` when `--mixxx-db` is omitted
...
[mixxx]        # optional — path to Mixxx DB
